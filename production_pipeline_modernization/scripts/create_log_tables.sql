...
/*
-- Example 1: Start a pipeline execution
DECLARE @ExecID INT;
EXEC sp_StartPipelineExecution
    @PipelineRunID = 'run-12345',
    @PipelineName = 'HR_Payroll_Pipeline',
    @SourceFileName = 'hr_payroll_data.zip',
    @SourceFileSize = 2048,
    @TriggerType = 'Event',
    @ExecutionID = @ExecID OUTPUT;

-- Example 2: Log an activity (start)
DECLARE @ActID INT;
EXEC sp_LogActivity
    @ExecutionID = @ExecID,
    @ActivityName = 'Validate_Data',
    @ActivityType = 'AzureFunction',
    @Status = 'Running',
    @InputParameters = '{"file":"hr_payroll_data.zip"}',
    @ActivityLogID = @ActID OUTPUT;

-- Example 3: Update activity (end)
EXEC sp_LogActivity
    @ActivityLogID = @ActID,
    @Status = 'Succeeded',
    @OutputParameters = '{"exitCode":0,"filesValidated":2}',
    @RecordsRead = 20;

-- Example 4: End pipeline execution
EXEC sp_EndPipelineExecution
    @ExecutionID = @ExecID,
    @Status = 'Succeeded',
    @RecordsProcessed = 20;
...
EXEC sp_LogError
    ...
    @SourceComponent = 'ValidationFunction';
*/
